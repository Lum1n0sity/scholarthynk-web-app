 ...
 */
export async function updateNote(authToken, noteTitle, originalTitle, noteContent, path, testing = false) {
    if (authToken) {
        ...
        if (response.status === 200) {
            ...
            throw new Error("Error while renaming note: Invalid input");
            ...
            throw new Error("Error while renaming note: Note not found");
            ...
            throw new Error("Error while renaming note: Unauthorized");
            ...
            throw new Error("Error while renaming note: Internal Server Error");
        } else {
 ...
 */
export async function getNote(authToken, path, title, isNewNote, testing = false) {
    if (authToken) {
        ...
        if (response.status === 200) {
            ...
            throw new Error("Error while loading note: Note not found");
        } else if (response.status === 401) {
            if (!testing) {
                const err = await response.json();
                newNotification("error", "Unauthorized", err.error);
            ...
            throw new Error("Error while loading note: Unauthorized");
        } else if (response.status === 500) {
            if (!testing) {
                const err = await response.json();
            ...
            throw new Error("Error while loading note: Internal Server Error");
        } else {
